...
public class CannonBlockScreenPacket implements CustomPacketPayload {

    public static final Type<CannonBlockScreenPacket> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(BlockySiege.MOD_ID, "cannon_screen_packet"));
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, CannonBlockScreenPacket> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, CannonBlockScreenPacket message) -> {
        buffer.writeBlockPos(message.blockPos);
        buffer.writeInt(message.eventType);
        buffer.writeInt(message.pressed);
    }, (RegistryFriendlyByteBuf buffer) -> new CannonBlockScreenPacket(buffer.readBlockPos(), buffer.readInt(), buffer.readInt()));

    BlockPos blockPos;
    public final int eventType;
    public final int pressed;

    public CannonBlockScreenPacket(BlockPos pos, int eventType, int pressedms) {
        this.blockPos = pos;
    ...
    public static void handleData(final CannonBlockScreenPacket message, final IPayloadContext context) {
    ...
    public static void pressAction(Player player, BlockPos pos, int type, int pressed) {
        ...
        switch (type) {
            ...
            case SET_POWER_1 -> {
                ...
                cannonBlockEntity.setChanged(); // Add this!
            }
            case SET_POWER_2 -> {
